...
veryOldInventors = inventors.filter((investor) => {
  return investor.year >= 1500 && investor.year <= 1599
})
// Check your work:
...
// Complete the exercise in the space below:
inventorNames = inventors.map((investor) => {
  return `first: ${investor.first}, last: ${investor.last}`
})
// Check your work:
...
// Complete the exercise in the space below:
sortedByBirthYear = inventors.sort((f, s) => f.year - s.year)
// Check your work:
...
firstLast = people.map((person) => {
  let letNewPerson = person.split(',', 2)
  return `${letNewPerson[1]} ${letNewPerson[0]}`
})
// Check your work:
